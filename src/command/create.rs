pub mod subscription {
    ...
    pub async fn run(
        ...
        let mut cmd = connection
            .create_persistent_subscription(stream_name, group_id)
            .settings(setts);
...
pub mod projection {
    ...
    use crate::api::{Api, ProjectionConf};
    ...
    pub async fn run(
        ...
        let script_filepath = params
            .value_of("SCRIPT")
            .expect("SCRIPT was check by clap already");
        let script = std::fs::read_to_string(script_filepath).map_err(|e| {
            CerberusError::UserFault(format!(
                "There was an issue with the script's filepath you submitted: {}",
                e
            ))
        })?;
        ...
        let conf = ProjectionConf {
            ...
            kind: params
                .value_of("kind")
                .expect("Kind was check by clap already"),
        ...
        let result = api.create_projection(conf).await?;
        let info = api.projection_cropped_info(&result.name).await?;

        if info.status == "Faulted" {
            let reason = info
                .reason
                .unwrap_or_else(|| "<unavailable faulted reason>".to_owned());

            return Err(CerberusError::user_fault(format!(
                "Unsuccessful projection [{}] creation:\n>> {}",
                result.name, reason
            )));
        }
